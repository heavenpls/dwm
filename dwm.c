...
#define ISSHOW(C)               ((C->tags & C->mon->tagset[C->mon->seltags]) && !C->ishidden)
...
static int drawstatusbar(Monitor *m, int bh, char* stext,int stw);
...
drawbar(Monitor *m)
{
	...
	int temp_x  = x,v_count = 0;
	if ((w = m->ww - tw - x) > bh) {
		// clean client name
		drw_setscheme(drw, scheme[SchemeNorm]);
		drw_rect(drw, x, 0, w, bh, 1, 1);	
		// judge title width logger than bar width ?	
			...
			for ( Client *c = m->clients; c ; c = c->next ){
				if ( ISVISIBLE(c) ){
					if ( c == m->sel ){
						...
						drw_setscheme(drw,scheme[SchemeNorm]);
					int width = TEXTW(c->name);
					drw_text(drw, x, 0, width > title_width ? title_width:width, bh, lrpad / 2, c->name, 0);
					x = x + (title_width > width ? width : title_width );
					if (m->sel->isfloating)
						drw_rect(drw, x + boxs, boxs, boxw, boxw, m->sel->isfixed, 0);
					}
...
nexttiled(Client *c)
{
	for (; c && (c->isfloating || !ISSHOW(c)); c = c->next);
	return c;
}
...
void grid(Monitor *m){
    int n = 0,x,y,w,h;
    Client *temp;
    for (temp = m->clients; temp ; temp = temp->next) {
        if (ISSHOW(temp))
            n++;
    }
    if ( n == 1 ){
        for (temp = m->clients; temp ; temp = temp->next) {
            if ( ISSHOW(temp) ){
                resize(temp, m->ww/4, m->wh/4, m->ww/2, m->wh/2, 0);	
                break;
            }
        }
    }
        ...
        for (temp = m->clients; temp ; temp = temp->next) {
            if ( ISSHOW(temp) ){
                resize(temp, x, y, m->ww/2, m->wh/2, 0);	
                x += m->ww/2;
            }
        }
    }
       ...
       int nw,ny,x = 0,y = 0,nn = 0;
       char ch[20];
       if ( (n_sqrt * n_sqrt) == n ){
           nw = ny = n_sqrt;
       }else {
           nw = n_sqrt + 1;
           float temp_num = (float) n / nw;
           ny = (int) ceil(temp_num);
       }
       w = m->ww/nw;
       h = m->wh/ny;
       for (temp = m->clients; temp ; temp = temp->next) {
            if ( ISSHOW(temp) ){
                resize(temp, x, y, w, h, 0);	
                nn++;
                x += w;
            }
            if ( nn == nw ){
...
#endif /* __OpenBSD__ */
	scan();
    system("/home/heaven/.config/dwm/script/script.sh &");
	run();
	cleanup();
	XCloseDisplay(dpy);
	return EXIT_SUCCESS;
